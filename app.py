...
st.title("🎙️ Dolat Nagar AI Voiceover & Dictation Studio")
st.write("Bol kar type karain, text theek karain aur premium AI voiceover download karain.")
...
if lang_choice == "Urdu (Pakistan)":
    text_placeholder = "Yahan Urdu likhein ya bolain..."
    ...
    voice_options = {"Mandeep (Female)": "pa-IN-MandeepNeural", "Gurpreet (Male)": "pa-IN-GurpreetNeural"}
...
# Voice Input Indicator (Instruction block for standard web devices)
st.info("💡 **Voice Typing Tip:** Agar aap Mobile ya Chrome browser use kar rahe hain, to apne keyboard par bane **Mic (Microphone) icon** par click kar ke bol kar asaani se mukammal text type kar sakte hain!")

user_text = st.text_area("Apna Text Yahan Likhein:", placeholder=text_placeholder, height=150)

# Clean Text Function (Simple punctuation and spacing automatic handler)
if st.button("🪄 Auto-Clean & Fix Text (Alfaz Durust Karain)"):
    if user_text.strip():
        # Clean double spaces and normalize common spacing errors
...
with col1:
    voice_selection = st.selectbox("Aawaz (Voice):", list(voice_options.keys()))
with col2:
